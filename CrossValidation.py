...
def cross_validation(X_train, Y_train, X_valid, Y_valid, layers_dims_list, lambda_l1_list, lambda_l2_list, hidden_layers_activation_fn="relu", with_momentum=True, learning_rate=0.01,  num_epochs=3000,
                     print_cost=False):
    ...
    for layers_dims in layers_dims_list:
        ...
        parameters, error, error_list, accuracy_list = model_with_regularization(X_train, Y_train, layers_dims,
        ...
        plotError(error_list, len(error_list), hidden_layers_activation_fn, with_momentum, model_name)
        plotAccuracy(accuracy_list, len(accuracy_list), hidden_layers_activation_fn, with_momentum, model_name)
    ...
    if reg_type=="no regularization":
        print("Best configuration is ", best_dim," using " + reg_type)
    else:
        print("Best configuration is ", best_dim," using " + reg_type, " with lambda", best_lambda)
    plotError(error_list_final_model, len(error_list_final_model), hidden_layers_activation_fn, with_momentum)
